...
export default function NewPostPage() {
  const [form, setForm] = useState({ title: '', slug: '', summary: '', content: '', status: 'DRAFT' })
  const [suggestedTags, setSuggestedTags] = useState<string[]>([])
  useEffect(() => {
    const s = form.title.toLowerCase().trim().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-')
    setForm(f => ({ ...f, slug: s }))
  }, [form.title])
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    const res = await fetch('/api/admin/posts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    ...
    <div className="container py-8">
      ...
      <form onSubmit={submit} className="bg-gray-800 rounded-2xl p-4 space-y-3">
        <div className="flex gap-2">
          ...
          <button type="button" onClick={suggestTitle} className="border border-white bg-white text-gray-900 rounded-xl px-3">AI Help</button>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={genSummary} className="border border-white bg-white text-gray-900 rounded-xl px-3">AI Help</button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={suggestTags} className="border border-white bg-white text-gray-900 rounded-xl px-3">AI Help</button>
          {suggestedTags.length > 0 && (
            <div className="text-sm text-white/70">{suggestedTags.join(', ')}</div>
          )}
        </div>
        <select className="w-full p-2 rounded bg-gray-900" value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
          <option value="DRAFT">DRAFT</option>
          <option value="PUBLISHED">PUBLISHED</option>
        </select>
        <button className="inline-flex items-center gap-2 border border-white bg-white text-gray-900 h-10 px-4 rounded-xl">Create</button>
      </form>
    </div>
  )
}
